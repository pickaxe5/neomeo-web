...
def _get_token(db: Session, project: Project) -> str:
    if project.github_connected_by_user_id is None:
        raise CollectionError("프로젝트에 연동된 GitHub 계정이 없습니다.")
    user = db.get(User, project.github_connected_by_user_id)
...
def _build_pr_events(project_id: uuid.UUID, db: Session, prs: list[dict], existing_ids: set[str]) -> list[RawEvent]:
    ...
    for pr in prs:
        ...
        created_id = f"pr:{number}:created"
...
def _build_issue_events(
    project_id: uuid.UUID, db: Session, issues: list[dict], existing_ids: set[str]
) -> list[RawEvent]:
    events: list[RawEvent] = []
    for issue in issues:
        number = issue["number"]
        ...
        created_id = f"issue:{number}:created"
        if created_id not in existing_ids:
            events.append(
                RawEvent(
                    **base_kwargs,
        ...
        closed_id = f"issue:{number}:closed"
...
def _build_review_comment_events(
    project_id: uuid.UUID, db: Session, comments: list[dict], existing_ids: set[str]
) -> list[RawEvent]:
    events: list[RawEvent] = []
    for comment in comments:
        github_id = f"review_comment:{comment['id']}"
...
def collect_project_events(db: Session, project: Project) -> int:
    """프로젝트 1건에 대해 PR·이슈·리뷰코멘트·커밋을 수집해 저장한다. 반환값은 새로 저장된 건수.
    실패 시 project.gh_last_error에 사유를 남기고 CollectionError를 올린다."""
    if not project.repo_full_name:
        raise CollectionError("연동된 레포가 없습니다.")

    try:
        token = _get_token(db, project)
        since = project.gh_last_collected_at or (
            datetime.now(timezone.utc) - timedelta(days=BACKFILL_WINDOW_DAYS)
        )

        with httpx.Client(timeout=15) as client:
            prs = github_client.list_pull_requests(client, token, project.repo_full_name, since)
            issues = github_client.list_issues(client, token, project.repo_full_name, since)
            comments = github_client.list_review_comments(client, token, project.repo_full_name, since)
            commits = github_client.list_commits(client, token, project.repo_full_name, since)

            new_events: list[RawEvent] = []
            new_events += _build_pr_events(
                project.id, db, prs, _existing_github_ids(db, project.id, EventType.PR)
            )
            new_events += _build_issue_events(
                project.id, db, issues, _existing_github_ids(db, project.id, EventType.ISSUE)
            )
            new_events += _build_review_comment_events(
                project.id, db, comments, _existing_github_ids(db, project.id, EventType.REVIEW_COMMENT)
            )
            new_events += _build_commit_events(
                ...
                db,
                commits,
                _existing_github_ids(db, project.id, EventType.COMMIT),
            )

        for event in new_events:
            db.add(event)

        project.gh_last_collected_at = datetime.now(timezone.utc)
